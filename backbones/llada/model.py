...
class LLaDALlamaBlock(LLaDABlock):
    ...
    behavior of Llama. In particular, the MLP uses the standard LLaMA gated form:
    ...
    def forward(
        ...
        if self._activation_checkpoint_fn is not None:
            ...
            x = self.ff_norm(x)
        gate, x_up = self.ff_proj(x), self.up_proj(x)
        # Keep the LLaMA block architecture explicit instead of routing through the
        # generic activation abstraction used by the sequential block.
        gate = F.silu(gate)
        x = gate * x_up
        x = self.ff_out(x)
        x = self.dropout(x)
...
class LLaDAModelLM(LLaDAPreTrainedModel):
    ...
    _no_split_modules = ["LLaDALlamaBlock"]
    # Transformers>=5.0 calls all_tied_weights_keys on the instance after loading.
    # This architecture handles tying via F.linear in forward(); no HF-managed tied keys.
    _tied_weights_keys = []

    @property
    def all_tied_weights_keys(self):
        # transformers>=5.3 calls .keys() on items from this; return empty dict.
        return {}
    ...
    def __init__(self, config: LLaDAConfig, model: Optional[LLaDAModel] = None, init_params: bool = False):
        ...
        if not model:
            ...
            # Keep architecture intact while allowing standalone CPU-first initialization.
    ...
    def get_output_embeddings(self):
        # In tied mode, logits are produced directly with F.linear against input embeddings.
        # Returning None prevents HF resize/save utilities from creating synthetic output heads.
        if self.config.weight_tying:
            return None
    ...
    def set_output_embeddings(self, value: torch.nn.Module):
        if self.config.weight_tying:
            raise ValueError("set_output_embeddings is unsupported when weight_tying=True")
    ...
    def resize_token_embeddings(self, new_num_tokens: int | None = None, pad_to_multiple_of: int | None = None, mean_resizing: bool = True):
        ...
        # HF updates vocab_size, but this architecture also relies on embedding_size
        # when reconstructing the backbone during from_pretrained.
        new_embedding_size = embeddings.num_embeddings
        self.config.embedding_size = new_embedding_size
        self.model.config.embedding_size = new_embedding_size
        return embeddings

    def tie_weights(self, *args, **kwargs):
        # No-op by design for this architecture:
        # tied logits use F.linear(x, wte.weight) in the forward pass.
        if self.config.weight_tying and "ff_out" in self.model.transformer:
            # Defensive cleanup for objects created by older versions/checkpoints.
            self.model.transformer.pop("ff_out")


# Standalone MDLM aliases that preserve the exact LLaDA architecture/classes.
