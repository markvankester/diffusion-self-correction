...
class LLaDAConfig(PretrainedConfig):
    ...
    def __init__(self, use_cache: bool = False, use_return_dict: bool = True, **kwargs):
        model_config = ModelConfig()
        all_kwargs = model_config.__dict__.copy()
        all_kwargs.update(kwargs)
        all_kwargs["use_cache"] = use_cache
        all_kwargs["return_dict"] = use_return_dict
        
        # We don't set self.use_return_dict because it's a read-only property in Transformers 5.x
        # Instead, we set return_dict which the property reads from.
        self.return_dict = use_return_dict
        self.use_cache = use_cache
        
        if "architectures" not in all_kwargs:
            all_kwargs["architectures"] = ["LLaDAModelLM"]
            
        super().__init__(**all_kwargs)
        
        # Enforce these one last time to be safe
        self.return_dict = use_return_dict
        self.use_cache = use_cache
...
# Alias for standalone MDLM naming while preserving the exact LLaDA config structure.
MDLMConfig = LLaDAConfig
